	...
	useEffect(() => {
		const unsubscribe = onSnapshot(
			...
			(snapshot: QuerySnapshot) => {
				...
				// Exclude "Dr Dharanjay Dubey" only when role is Admin (so Admin doesn't appear in Referred by)
...
const handleRegisterPatient = async (event: React.FormEvent<HTMLFormElement>) => {
	...
	const handleDownloadReportPDF = async (sections?: ReportSection[]) => {
		...
		await generatePhysiotherapyReportPDF({
			...
			history: (reportPatientData as any).history || ((reportPatientData as any).presentHistory || '') + ((reportPatientData as any).pastHistory ? '\n' + (reportPatientData as any).pastHistory : ''),
