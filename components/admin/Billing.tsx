...
import { collection, doc, onSnapshot, updateDoc, setDoc, deleteDoc, query, where, getDocs, addDoc, serverTimestamp, orderBy, writeBatch, type QuerySnapshot, type Timestamp } from 'firebase/firestore';
...
import { getCurrentBillingCycle, getNextBillingCycle, getBillingCycleId, getMonthName, getCurrentCalendarYear, type BillingCycle } from '@/lib/billingUtils';
...
export default function Billing() {
				...
				const mapped = snapshot.docs.map(docSnap => {
					...
					return {
						...
						sessionAllowance: data.sessionAllowance
							...
							: undefined,
					} as AdminPatientRecord & { id: string; department?: string; packageAmount?: number; totalSessionsRequired?: number };
				});
				setPatients([...mapped]);
			},
			error => {
						...
						amountPaid:
							typeof data.amountPaid === 'number'
								...
								: data.amountPaid
									...
									: undefined,
						packageSessions:
							typeof data.packageSessions === 'number'
								? data.packageSessions
								: data.packageSessions
									? Number(data.packageSessions)
									: undefined,
						date: data.date ? String(data.date) : '',
		...
		try {
			...
			for (const row of validRows) {
				...
				batch.set(billingRef, {
					...
					paymentMode: 'N/A',
					utr: null,
					paymentRegisteredByFrontdeskName: 'N/A',
					createdAt: serverTimestamp(),
					updatedAt: serverTimestamp(),
				});
			}
	...
	// Generate receipt HTML when selectedBill changes
	useEffect(() => {
		if (selectedBill && showPaymentSlipModal) {
			const receiptNo = selectedBill.billingId || `BILL-${selectedBill.id?.slice(0, 8) || 'NA'}`;
			generateReceiptHtml(selectedBill, receiptNo).then(setReceiptHtml).catch(error => {
	...
	}, [selectedBill, showPaymentSlipModal]);
	...
	// Pending payments from billing collection (include all pending so package patients appear)
	const pending = useMemo(() => {
		return billing.filter(bill => bill.status === 'Pending');
	}, [billing]);
	...
	const filteredCompleted = useMemo(() => {
		...
		return completed.filter(bill =>
			...
			bill.createdByFrontdeskName?.toLowerCase().includes(q) ||
			bill.paymentRegisteredByFrontdeskName?.toLowerCase().includes(q)
		);
	...
	const pendingDisplayRows = useMemo((): BillingDisplayRow[] => {
		...
		const billsForPackagePatients = filteredPending.filter(b => b.patientId && packagePatientIds.has(b.patientId));
		const billsForNonPackagePatients = filteredPending.filter(b => !b.patientId || !packagePatientIds.has(b.patientId));
		...
		byPatient.forEach((bills, patientId) => {
			...
			const totalSessions = patient?.totalSessionsRequired ?? 0;
		...
		return rows;
	}, [filteredPending, patientLookup, appointments, billingDisplayInstallments, packagePatientIds]);
	...
	const completedDisplayRows = useMemo((): BillingDisplayRow[] => {
		...
		return rows;
	}, [filteredCompleted, patientLookup, appointments, billingDisplayInstallments, packagePatientIds]);
	...
	return (
		<div className="min-h-svh bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 px-6 py-10">
			...
			<section className="mx-auto mt-4 grid max-w-6xl gap-4">
				<article className="flex flex-col rounded-2xl bg-white shadow-[0_18px_40px_rgba(15,23,42,0.07)] max-h-[calc(100vh-280px)]">
					<header className="flex flex-col rounded-t-2xl bg-amber-100 px-4 py-3 text-amber-900 flex-shrink-0">
						...
						<div className="relative mt-2">
							<input
								...
								className="w-full rounded-lg border border-amber-300 bg-white px-3 py-1.5 pl-8 text-xs text-slate-900 placeholder-slate-400 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-200"
							/>
							<i className="fas fa-search absolute left-2 top-1/2 -translate-y-1/2 text-slate-400 text-xs" aria-hidden="true" />
						</div>
					</header>
					<div className="overflow-x-auto overflow-y-auto px-4 pb-4 pt-2 flex-1">
						<table className="min-w-full divide-y divide-amber-200 text-left text-xs text-slate-700">
							...
							<tbody className="divide-y divide-amber-100">
								{pendingDisplayRows.length === 0 ? (
									...
									pendingDisplayRows.map((row) => {
										const { bill, packageInfo } = row;
										const rowKey = packageInfo ? `pkg-${bill.patientId}-${packageInfo.installmentIndex}-${packageInfo.installmentTotal}` : (bill.id || bill.billingId);
										return (
										<tr key={rowKey}>
											<td className="px-2 py-2 text-xs font-medium text-slate-800">
												<div className="flex flex-col gap-0.5">
													<div className="flex items-center gap-1.5">
														...
														{packageInfo && (
															<span className="inline-flex items-center rounded-full bg-indigo-100 px-1.5 py-0.5 text-[10px] font-semibold text-indigo-800">PKG</span>
														)}
													</div>
													{packageInfo && (
														<span className="text-[10px] text-slate-500">{packageInfo.completedSessions}/{packageInfo.totalSessions} sessions</span>
													)}
												</div>
											</td>
											...
											<td className="px-2 py-2 text-right">
												<div className="flex items-center justify-end gap-1.5">
													{packageInfo && (
														<button
															type="button"
															onClick={() => { setInstallmentModal({ patientId: packageInfo.patientId, patientName: packageInfo.patientName }); setInstallmentInput(String(packageInfo.installmentTotal || 1)); }}
															className="inline-flex items-center rounded-lg border border-slate-300 px-2 py-0.5 text-[10px] font-semibold text-slate-700 transition hover:bg-slate-50"
														>
															Installment
														</button>
													)}
			...
			<section className="mx-auto mt-4 grid max-w-6xl gap-4">
				<article className="flex flex-col rounded-2xl bg-white shadow-[0_18px_40px_rgba(15,23,42,0.07)] max-h-[calc(100vh-280px)]">
					<header className="flex flex-col rounded-t-2xl bg-emerald-100 px-4 py-3 text-emerald-900 flex-shrink-0">
						<div className="flex items-center justify-between mb-2">
							<div>
							...
							<div className="flex items-center gap-2">
								...
								<button
									...
									className="inline-flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-emerald-600 via-emerald-700 to-teal-600 px-3 py-1.5 text-xs font-semibold text-white shadow-md hover:from-emerald-700 hover:via-emerald-800 hover:to-teal-700 transition-all duration-200 hover:scale-105"
								>
									<i className="fas fa-file-excel text-[10px]" aria-hidden="true" />
									Import Excel
								</button>
							</div>
						</div>
						<div className="relative mt-2">
							<input
								type="text"
								placeholder="Search by Patient, Patient ID, Doctor, Amount, Date, Payment Mode, Created By, or Payment By..."
								value={completedSearchQuery}
								onChange={(e) => setCompletedSearchQuery(e.target.value)}
								className="w-full rounded-lg border border-emerald-300 bg-white px-3 py-1.5 pl-8 text-xs text-slate-900 placeholder-slate-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-200"
							/>
							<i className="fas fa-search absolute left-2 top-1/2 -translate-y-1/2 text-slate-400 text-xs" aria-hidden="true" />
						</div>
					</header>
										...
										<tr key={rowKey}>
												<td className="px-2 py-2 text-xs font-medium text-slate-800">
													<div className="flex flex-col gap-0.5">
														...
														{patient?.department && !packageInfo && (
							...
							{receiptHtml ? (
								...
								<div className="flex items-center justify-center py-8">
									<div className="text-center">
										...
										<p className="text-sm text-slate-600">Loading receipt...</p>
									</div>
								</div>
							)}
						</div>
							...
							{!importFile ? (
								<div className="border-2 border-dashed border-slate-300 rounded-lg p-8 text-center">
									<input
										...
										className="hidden"
										id="excel-file-input-admin"
									/>
									<label
										htmlFor="excel-file-input-admin"
